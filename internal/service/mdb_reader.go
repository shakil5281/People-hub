...
$conn = $null
$tempMDB = Join-Path $env:TEMP ("peoplehub_read_" + [guid]::NewGuid().ToString() + ".mdb")

try {
  # Copy MDB to temp location to completely bypass any process locks
  Copy-Item -Path '%s' -Destination $tempMDB -Force -ErrorAction Stop
  ...
  $conn.Open("Provider=Microsoft.ACE.OLEDB.12.0;Data Source='$tempMDB'")
