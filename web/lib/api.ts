...
export const attendanceApi = {
  ...
  late: (params?: Record<string, string>) => api.get("/attendance/late", { params }),
  fixSingleLate: (data: Record<string, unknown>) => api.post("/attendance/fix-single-late", data),
...
export const salaryIncrementApi = {
  ...
  delete: (id: string) => api.delete(`/salary/increments/${id}`),
}

export const advanceSalaryApi = {
  list: (params?: Record<string, string>) => api.get("/salary/advances", { params }),
  bulkApply: (data: Record<string, unknown>) => api.post("/salary/advances/bulk-apply", data),
  approve: (id: string, data?: Record<string, unknown>) => api.put(`/salary/advances/${id}/approve`, data || {}),
  reject: (id: string, data?: { reason: string }) => api.put(`/salary/advances/${id}/reject`, data || {}),
  delete: (id: string) => api.delete(`/salary/advances/${id}`),
  exportExcel: (params?: Record<string, string>) => api.get("/salary/advances/export/excel", { params, responseType: "blob" }),
...
export const zktecoSyncApi = {
  getStatus: (params?: Record<string, string>) => api.get<{ mdb_path: string; mdb_exists: boolean; total: number; synced_count: number; data: Array<{ id: string; employee_id: string; punch_number: string; name: string; department: string; designation: string; is_synched: boolean }> }>("/admin/zkteco-sync/status", { params }),
  sync: (data: { mdb_path?: string; employee_ids?: string[]; sync_all?: boolean }) => api.post<{ success: boolean; synced_count: number; failed_count: number; total: number; message: string; details: Array<{ employee_id: string; punch_number: string; name: string; status: string; message: string }> }>("/admin/zkteco-sync", data),
  testConnection: (data: { mdb_path?: string }) => api.post<{ connected: boolean; user_count?: number; message: string }>("/admin/zkteco-sync/test-connection", data),
}
