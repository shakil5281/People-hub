...
import { TimerIcon, ArrowLeftIcon, UsersIcon } from "lucide-react"
...
import { bulkCreateTempShift } from "@/components/data/temporary-shift-data"
...
export default function TemporaryShiftCreatePage() {
  ...
  const handleSubmit = async () => {
    ...
    setSubmitting(true)
    try {
      const ok = await bulkCreateTempShift(payload)
      if (ok) {
        toast.success(`Temporary shift assigned to ${employee_ids.length} employees for ${format(date, "yyyy-MM-dd")}`)
        setDialogOpen(false)
        setSelectedRows([])
        router.push("/information/temporary-shift")
      } else toast.error("Failed to assign temporary shift")
    } catch (err: unknown) {
      const axiosErr = err as { response?: { data?: { error?: string } } }
      toast.error(axiosErr.response?.data?.error || "Failed to assign temporary shift")
    } finally {
      setSubmitting(false)
    }
  }
    ...
    <div className="flex flex-col gap-4 py-4 md:gap-6 md:py-6">
      <div className="px-4 lg:px-6 flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <div>
            <h1 className="text-3xl font-bold tracking-tight">Add Temporary Shift — 1 Day</h1>
            <p className="text-muted-foreground mt-1">Advance filter → select active employees → assign shift for single date (all employees share same date)</p>
          </div>
        </div>
        <div className="flex items-center gap-2">
          ...
          {selectedRows.length > 0 && (
            <Button onClick={handleOpenDialog}>
              <TimerIcon className="mr-2 h-4 w-4" /> Assign Shift ({selectedRows.length})
            </Button>
          )}
        </div>
      </div>
      ...
      <div className="px-4 lg:px-6 space-y-3">
        <p className="text-sm text-muted-foreground">Showing active employees — select via checkbox or Select All to assign the same shift for a single date. {selectedRows.length > 0 && <span className="font-medium text-foreground">{selectedRows.length} selected — click Assign Shift to open dialog</span>}</p>
        <DataTable
          data={employees}
          columns={columns}
          serverSide={true}
      ...
      <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
        <DialogContent className="max-w-lg">
          <DialogHeader>
            <DialogTitle>Assign Temporary Shift — 1 Day</DialogTitle>
            <DialogDescription>Assign shift to {selectedRows.length} selected employee(s) for a single date.</DialogDescription>
          </DialogHeader>
          <div className="space-y-4">
            ...
            <div className="space-y-2">
              <Label>Date *</Label>
              <DatePicker value={date} onChange={setDate} placeholder="Select date (single day)" />
              <p className="text-xs text-muted-foreground">Only one date — all employees will share this temporary shift date.</p>
            </div>
            <div className="space-y-2">
              <Label>Reason</Label>
              <Input placeholder="Replacement, Overtime, Emergency..." value={reason} onChange={(e) => setReason(e.target.value)} />
            </div>
