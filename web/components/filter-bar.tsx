  ...
  return (
    <div className={noBorder ? "p-4" : "rounded-lg border bg-card p-4"}>
      <div className={singleColumn ? "grid grid-cols-1 gap-3" : "grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3"}>
        {filters.map((f) => {
          if (f.type === "daterange-split") {
            return (
              <React.Fragment key={f.key}>
                <div className="flex flex-col gap-1.5 lg:hidden">
                  <label className="text-xs font-medium text-muted-foreground">{f.label} (Start)</label>
                  <DatePicker
                ...
                <div className="flex flex-col gap-1.5 lg:hidden">
                  <label className="text-xs font-medium text-muted-foreground">{f.label} (End)</label>
                  <DatePicker
